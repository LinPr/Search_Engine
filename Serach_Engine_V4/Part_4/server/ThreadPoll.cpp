...
ThreadPool::ThreadPool(int threadsNum, int queSize, int CacheCapacity)
: _threadsNum(threadsNum)
, _threads()
, _taskQueue(queSize)
, _isExit(false)
, _cachesManager(CacheCapacity)
{
...
{   
    ...
    {   
        ...
        // /* 将缓存跟每一个线程，通过pthread_id 关联起来 */
        _cachesManager.addCache(_threads[i]->get_pthreadID());
