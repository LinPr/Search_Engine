...
std::vector<WebPage> & WebSource::get_webPageLib()
{
    return _webPageLib;
}

std::map<pageID, std::pair<pageStart, pageOffset>> & WebSource::get_pagesOffsetMap()
{
    return _pagesOffsetMap;
}

std::unordered_map<std::string, std::vector<std::pair<pageID, wordStrength>>> & WebSource::get_invertIndex()
{
    return _invertIndex;
}
